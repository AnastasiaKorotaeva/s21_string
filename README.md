...
4. [Информация](#Информация)  
    4.1. [string.h Типы](#stringh-Типы) \
    4.2. [string.h Макросы](#stringh-Макросы) \
    4.3. [string.h Функции](#stringh-Функции) \
    4.4. [sprintf](#sprintf) \
    4.5. [sprintf Спецификаторы](#sprintf-Спецификаторы) \
    4.6. [sprintf Флаги](#sprintf-Флаги) \
    4.7. [sprintf Ширина](#sprintf-Ширина) \
    4.8. [sprintf Точность](#sprintf-Точность) \
    4.9. [sprintf Длина](#sprintf-Длина) \
    4.10. [Специальные функции обработки строк (вдохновленные классом String в языке C#)](#Специальные-функции-обработки-строк-вдохновленные-классом-String-в-языке-C) 
5.  [Реализация функций библиотеки string.h](#Реализация-функций-библиотеки-stringh)  
    5.1. [Частичная реализация функции sprintf](#Частичная-реализация-функции-sprintf) \
    5.2. [Реализация некоторых модификаторов формата функции sprintf](#Реализация-некоторых-модификаторов-формата-функции-sprintf) \
    5.3. [Реализация специальных функций обработки строк](#Реализация-специальных-функций-обработки-строк)


## Краткое описание

В данном проекте разработана собственная реализация библиотеки string.h на языке программирования Си с некоторыми дополнениями (с собственной реализацией функций sprintf). Библиотека string.h является основной библиотекой языка Си по обработке строк.

## Команда разработки проекта

Над реализацией проекта работали: \
[tamramar](https://github.com/AnastasiaKorotaeva) Анастасия \
[kidneyha](https://github.com/Alicardi) Олег \
craydlec  Раниль                                    
tuppngan  Роберт   
...
`make all` - Эта цель запускает цель `s21_string.a`

`make test` - Эта цель компилирует и запускает тестовый файл s21_string_test.c, который,содержит набор тестов для библиотеки s21_string.a

`make s21_string.a` - Эта команда создает статическую библиотеку из исходных файлов
...
1. Реализованы описанные [выше](#stringh-функции) функции библиотеки string.h: 

- Библиотека разработана на языке Си стандарта C11 с использованием компилятора gcc
- Оформлена как статическая библиотека (с заголовочным файлом s21_string.h)
...
Реализована функция sprintf из библиотеки stdio.h с поддержкой:
...
Реализованы некоторые модификаторы формата функции sprintf из библиотеки stdio.h с поддержкой:
